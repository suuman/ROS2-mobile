...
/**
 * A transformation in terms of translation and rotation. Ported from
 * rosjava_geometry and adapted to the plain ROS 2 message classes used with
...
public class Transform {
    ...
    private final Quaternion rotationAndScale;
    ...
    public static Transform fromTransformMessage(geometry_msgs.Transform message) {
        return new Transform(Vector3.fromVector3Message(message.translation),
                Quaternion.fromQuaternionMessage(message.rotation));
    }

    public static Transform fromPoseMessage(geometry_msgs.Pose message) {
        return new Transform(Vector3.fromPointMessage(message.position),
                Quaternion.fromQuaternionMessage(message.orientation));
    }
     ...
     *
     * @param other the {@link Transform} to apply to this {@link Transform}
    ...
    /**
     * @return the 4x4 transformation matrix of this {@link Transform} in column
    ...
    public geometry_msgs.Transform toTransformMessage(geometry_msgs.Transform result) {
        translation.toVector3Message(result.translation);
        rotationAndScale.toQuaternionMessage(result.rotation);
        return result;
    }

    public geometry_msgs.Pose toPoseMessage(geometry_msgs.Pose result) {
        translation.toPointMessage(result.position);
        rotationAndScale.toQuaternionMessage(result.orientation);
        return result;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (rotationAndScale == null) {
            if (other.rotationAndScale != null) return false;
